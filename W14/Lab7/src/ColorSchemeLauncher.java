...
public class ColorSchemeLauncher {
	...
	public void drawScheme(ColorScheme scheme, int y) {
		...
		for (Color c : colors) {
			this.g.setColor(c);
			this.g.fillRect(0+(100*count), y, 25, 25);
			this.g.fillRect(0+50+(100*count), y, 25, 25);
			this.g.fillRect(0+25+(100*count), y+25, 25, 25); 
			//comment in above line and comment out below line for cooler version.
			//this.g.fillRect(0+75+(100*count), y+25, 25, 25); 
			this.g.fillRect(0+(100*count), y+50, 25, 25);
			this.g.fillRect(0+50+(100*count), y+50, 25, 25);
			count= count+1;
		}
	}
